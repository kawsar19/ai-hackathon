...
export default function ApprovedProjectsPage() {
  ...
  const fetchIdeas = async () => {
    try {
      ...
      const approvedOnly: Idea[] = (data.ideas || []).filter((i: Idea) => String(i.status).toUpperCase() === 'APPROVED')
      setIdeas(approvedOnly)
    } catch (e) {
      console.error(e)
  ...
  const moveToInProgress = async (ideaId: string) => {
    try {
      setMovingId(ideaId)
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-bold text-gray-900">Approved Projects</h1>
          <p className="text-gray-600 text-xs">Projects approved and ready to start</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          ...
          <div key={project.id} className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow">
            <div className="p-6">
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-xs text-gray-500">Approved on {new Date(project.updatedAt).toLocaleString()}</span>
              </div>
              ...
              <div className="flex space-x-2 mt-3">
                ...
                <button
                  onClick={() => moveToInProgress(project.id)}
                ...
                >
                  {movingId === project.id ? (
                    <>
                      <Loader2 className="h-4 w-4 inline animate-spin mr-1" /> Moving
                    </>
                  ) : (
                    <>Move to In Progress</>
                  )}
        ...
        {filtered.length === 0 && (
          <div className="text-sm text-gray-500">No approved projects yet.</div>
        )}
      </div>
            ...
            <div className="mt-3">
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">Approved on {new Date(selectedProject.updatedAt).toLocaleString()}</p>
                </div>
